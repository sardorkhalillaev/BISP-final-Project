...
class ModelHelper:
    def __init__(self, models_dir="models"):
        ...
        self.models = {}
        # Your model_files dictionary mapping keys to .pkl filenames
        self.model_files = {
    ...
    def _load_all_models_on_init(self):
        # ... (your existing robust loading logic) ...
    ...
    # Prediction methods (ensure they exist for each model key)
    def predict_departure_delay(self, features_df):
        ...
        return [np.nan] # Return a list with NaN or handle error appropriately
    ...
    def predict_gate(self, features_df): # Returns encoded labels
        model = self.models.get("gate_classifier")
        if model and not features_df.empty: return model.predict(features_df)
        if features_df.empty : print("Warning: Empty features_df passed to predict_gate")
        return [np.nan] # Or appropriate placeholder for "unknown gate"

    def predict_on_time_performance(self, features_df): # Returns (encoded_class, proba)
        ...
        return ([np.nan], np.array([[np.nan, np.nan]])) # Placeholder for empty

    def forecast_passenger_volume(self, features_df): # For daily total
    ...
    def estimate_weather_impact(self, features_df): # Returns encoded labels if classification
